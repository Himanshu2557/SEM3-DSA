#include <iostream>                   // Himanshu Goyal
using namespace std;                  // 21105029
                                      // ECE

class Node{  //Creating a node class
    public:
    //Creating objects within class
    string name;
    int age;
    Node*next; //A node pointer for next node
    Node*prev; //A node pointer for previous node
    //calling constructor to add data
...
//FUNCTION TO INSERT NEW DATA AT END OF LINKED LIST
void append(Node* &head,string name,int age){
    //Creating a node pointer to store address of head in temp
    Node*temp=head;
    //creating a new node to store name and age in it
    Node*new_node=new Node(name,age);
    //Inserting a node in empty list
    if(temp==NULL){
        head=new_node;
    }
...
void insert_at_head(Node*&head,string name,int age){
    //Creating a Node pointer to store address of new node in it 
    Node*new_node=new Node(name,age);
...
int main(){
    ...
    //Asking for number of family members
	cout<<"Enter Number Of Family Members:";cin>>no_of_family_mem;
    //Inserting family members details 
